...
Shader "Demo/102 Transparent Map"
{
	...
	Properties
	{
		...
		// Alpha Map
		_AlphaMap("Transparency Map (A)", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 fragmentCode(fragmentIn fragIn) : COLOR
			{
				...
				float4 alphaTex = tex2D(_AlphaMap, fragIn.tex.xy);
				float alphaValue = alphaTex.a * _DiffuseColor.a;
				...
				fragIn.fragCol = tex * float4(_AmbientColor.xyz + diffuseTerm + specularTerm + rimTerm, alphaValue);
