...
// Shader 		: 	Multipe Lights (Multi-Pass)
...
//
//					On top of all above shading, this shader adds support for
//					contribution of multiple lights in the scene.
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 fragmentCode(fragmentIn fragIn) : COLOR
			{
				...
				// Diffuse Term
				// This is basically N.L contribution
				float3 diffuseTerm = _DiffuseColor * _LightColor0.xyz * max(dot(fragIn.fragNorm, lightDirection),0.0);
				...
				float3 specularTerm = _SpecularColor * _LightColor0.xyz * pow(max(dot(lightReflectDir, viewDirection),0.0),_Shininess) * max(dot(fragIn.fragNorm, lightDirection),0.0);
				...
				fragIn.fragCol = float4(_AmbientColor.xyz + diffuseTerm + specularTerm + rimTerm, 1.0);
		...
		Pass
		{
			...
			float4 fragmentCode(fragmentIn fragIn) : COLOR
			{
				...
				{
					float3 pixelToLightDir = (_WorldSpaceLightPos0 - fragIn.posWorld).xyz;
					float distance = length(pixelToLightDir);
					attenuation = max(1.0/distance, 0.0);
					lightDirection = normalize(pixelToLightDir);
				}
				// Diffuse Term
				// This is basically N.L contribution
				...
				// Final color contribution by adding all terms
				fragIn.fragCol = float4(diffuseTerm, 1.0) * attenuation;
